...
try:
    ...
    )  # prefer modelscope if available
except Exception:
    ...
    def __init__(
        ...
        try:
            cache_dir = os.path.join(
                os.path.abspath(self.model_dir), "hf_cache"
            )
        ...
        if self.is_fp16:
            ...
            self.gpt.post_init_gpt2_config(
                use_deepspeed=use_deepspeed, kv_cache=True, half=False
            )
        ...
        w2v_name = "facebook/w2v-bert-2.0"
        ...
        self.semantic_model, self.semantic_mean, self.semantic_std = (
            build_semantic_model(
                os.path.join(self.model_dir, self.cfg.w2v_stat), w2v_name
            )
        )
        ...
        local_semantic_ckpt = os.path.join(
        ...
        if os.path.exists(local_semantic_ckpt):
            ...
            print(
                f"[IndexTTS2] Using local semantic codec: {semantic_code_ckpt}"
            )
        else:
            print(
                "[IndexTTS2] Downloading semantic codec from HF (amphion/MaskGCT)…"
            )
        ...
        print(
            ">> semantic_codec weights restored from: {}".format(
                semantic_code_ckpt
            )
        )
        ...
        local_campplus_ckpt = os.path.join(
            self.model_dir, "campplus_cn_common.bin"
        )
        if os.path.exists(local_campplus_ckpt):
            campplus_ckpt_path = local_campplus_ckpt
            print(
                f"[IndexTTS2] Using local campplus checkpoint: {campplus_ckpt_path}"
            )
        else:
            print(
                "[IndexTTS2] Downloading campplus checkpoint (funasr/campplus)…"
            )
        ...
        if used_local_bigvgan is not None:
            ...
            self.bigvgan = bigvgan.BigVGAN.from_pretrained(
                used_local_bigvgan, use_cuda_kernel=False
            )
        else:
            self.bigvgan = bigvgan.BigVGAN.from_pretrained(
        ...
        self.bpe_path = os.path.join(
            self.model_dir, self.cfg.dataset["bpe_model"]
        )
        ...
        mel_fn_args = {
            "n_fft": self.cfg.s2mel["preprocess_params"]["spect_params"][
                "n_fft"
            ],
            ...
            "num_mels": self.cfg.s2mel["preprocess_params"]["spect_params"][
                "n_mels"
            ],
            "sampling_rate": self.cfg.s2mel["preprocess_params"]["sr"],
            "fmin": self.cfg.s2mel["preprocess_params"]["spect_params"].get(
                "fmin", 0
            ),
            "fmax": (
                None
        ...
        self.model_version = (
            self.cfg.version if hasattr(self.cfg, "version") else None
        )
    ...
    def remove_long_silence(
        ...
        for i in range(0, codes.shape[0]):
            ...
            if not torch.any(code == self.stop_mel_token).item():
                ...
                stop_mel_idx = (code == self.stop_mel_token).nonzero(
                    as_tuple=False
                )
                len_ = (
                    stop_mel_idx[0].item()
                    if len(stop_mel_idx) > 0
                    else code.size(0)
                )
        ...
        if isfix:
            if len(codes_list) > 1:
                codes = pad_sequence(
                    codes_list,
                    batch_first=True,
                    padding_value=self.stop_mel_token,
                )
            else:
    ...
    def infer(
        ...
        **generation_kwargs,
        ...
        if (
            ...
            ref_target_lengths = torch.LongTensor([ref_mel.size(2)]).to(
                ref_mel.device
            )
        ...
        if emo_vector is not None:
            ...
            weight_vector = torch.tensor(emo_vector, dtype=torch.float32).to(
                self.device
            )
            ...
            if verbose:
                try:
                    print(
                        f"[QWEN/Vector] weights={weight_vector.detach().cpu().tolist()} sum_raw={float(wsum_raw):.3f} sum_clamped={float(wsum):.3f}"
                    )
                except Exception:
                    pass
            ...
            with torch.no_grad():
                with torch.amp.autocast(
                    ...
                    if emo_vector is not None:
                        ...
                        wsum = torch.clamp(
                            torch.sum(weight_vector), min=0.0, max=1.0
                        )
                    ...
                    codes, speech_conditioning_latent = (
                        self.gpt.inference_speech(
                            ...
                            cond_lengths=torch.tensor(
                                [spk_cond_emb.shape[-1]],
                                device=text_tokens.device,
                            ),
                            emo_cond_lengths=torch.tensor(
                                [emo_cond_emb.shape[-1]],
                                device=text_tokens.device,
                            ),
                            emo_vec=emovec,
                            do_sample=True,
                ...
                for code in codes:
                    if self.stop_mel_token not in code:
                        ...
                        len_ = (code == self.stop_mel_token).nonzero(
                            as_tuple=False
                        )[0] + 1
                        code_len = len_ - 1
                ...
                if verbose:
                    print(codes, type(codes))
                    print(
                        f"fix codes shape: {codes.shape}, codes type: {codes.dtype}"
                    )
                    print(f"code len: {code_lens}")

                m_start_time = time.perf_counter()
                use_speed = (
                    torch.zeros(spk_cond_emb.size(0))
                    .to(spk_cond_emb.device)
                    .long()
                )
                with torch.amp.autocast(
                    text_tokens.device.type,
                    ...
                    latent = self.gpt(
                        ...
                        codes,
                        torch.tensor(
                            [codes.shape[-1]], device=text_tokens.device
                        ),
                        emo_cond_emb,
...
class QwenEmotion:
    ...
    def convert(self, content):
        ...
        # 按照期望顺序重新排列
        ordered_parts = [
            parts_dict[key] for key in desired_order if key in parts_dict
        ]
        parts = ordered_parts
